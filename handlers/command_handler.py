...
from config_state import get_cached_config, update_config

async def command_handler_func(event, client):
    """
    ...
    if text.startswith('/del'):
        try:
            ...
            if 0 <= task_id < len(tasks):
                ...
                await update_config(client, config)
    ...
    if text.startswith('/add'):
        ...
        # Try to join source
        joined = await join_channel(client, task['source'])
        if not joined:
...
def register_command_handler(client):
    ...
    async def wrapper(event):
        await command_handler_func(event, client)
